...
def test_cli_stats_json_output() -> None:
    ...
    assert '"alignment_length": 1042' in result.output
    assert '"constant_sites": {' in result.output


def test_cli_stats_json_output_format_writes_json_file_even_with_txt_suffix(tmp_path: Path) -> None:
    runner = CliRunner()
...
def test_cli_stats_invalid_output_extension_exits_one(tmp_path: Path) -> None:
    runner = CliRunner()
    ...
    result = runner.invoke(
        ...
        [
            ...
            str(TEST_DATA / "test" / "EOG090X0971.faa"),
            "--output",
            str(tmp_path / "stats.bad"),
        ],
    )

    assert result.exit_code == 1
    assert "unsupported output extension" in result.output.lower()
...
def test_directory_csv_output_writes_summary_and_adjacent_per_gene(tmp_path: Path) -> None:
    runner = CliRunner()
...
def test_directory_txt_per_gene_defaults_to_adjacent_csv(tmp_path: Path) -> None:
    runner = CliRunner()
...
def test_cli_help_explains_options() -> None:
    ...
    assert "Write full results to a file" in result.output
...
def test_directory_per_gene_txt_output_writes_adjacent_table(tmp_path: Path) -> None:
    runner = CliRunner()
    output = tmp_path / "stats.txt"
    per_gene_output = tmp_path / "stats.per-gene.csv"
    ...
    assert "[per_gene]" not in content
...
def test_single_file_text_output_file_includes_per_taxon_table(tmp_path: Path) -> None:
    runner = CliRunner()
    output = tmp_path / "out.txt"
    ...
    assert result.exit_code == 0
    content = output.read_text()
    assert "[per_taxon]" in content
    assert "name,raw_length,ungapped_length,gap_ratio,ambiguous_ratio,gap_ambiguous_ratio" in content
