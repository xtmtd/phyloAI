...
**Files:**
- Modify: `phyloai/core/env.py:37`
- Modify: `tests/cli/test_doctor.py:10`

- [ ] **Step 1: Extend `TOOL_REGISTRY` after `pb_mpi`**
...
```bash
git add phyloai/core/env.py tests/cli/test_doctor.py
git commit -m "feat: add PhyloBayes MPI tools to doctor registry"
```
...
```bash
git add phyloai/tree/bi.py tests/tree/test_bi.py
git commit -m "feat: add tree bi pure helpers"
```
...
def _build_chain_cmd(mpirun: str, pb_mpi: str, threads: int, matrix: Path, model_flags: list[str], sample_freq: int, nsamples: int, chain_name: str) -> list[str]:
    ...
    cmd = [mpirun, "-np", str(threads), pb_mpi, "-d", str(matrix.resolve()), *model_flags, "-x", str(sample_freq)]
    if nsamples != -1:
        if nsamples < 1:
            raise ValueError("--nsamples must be -1 or a positive integer")
        cmd.append(str(nsamples))
...
```bash
git add phyloai/tree/bi.py tests/tree/test_bi.py
git commit -m "feat: add tree bi tool resolution and command builders"
```
...
```bash
git add phyloai/tree/bi.py tests/tree/test_bi.py
git commit -m "feat: add tree bi run state handling"
```
...
```bash
git add phyloai/tree/bi.py tests/tree/test_bi.py
git commit -m "feat: add tree bi convergence parsers and trace plotting"
```
...
```python
def _run_convergence_check(output_dir: Path, chain_names: list[str], tools: dict[str, str], burnin: int, no_plot: bool) -> dict[str, Any]:
    ...
    for label, names in comparisons.items():
        ...
        bp_cmd = [tools["bpcomp"], "-x", str(burnin), "-o", str(Path("convergence") / base), *[str(Path("chains") / name) for name in names]]
        subprocess.run(bp_cmd, cwd=output_dir, capture_output=True, text=True)
        ...
        trace_proc = subprocess.run(trace_cmd, cwd=output_dir, capture_output=True, text=True)
...
```bash
git add phyloai/tree/bi.py tests/tree/test_bi.py
git commit -m "feat: add tree bi convergence runner"
```
...
- [ ] **Step 1: Add result assembly and dry-run capable `run_bi()`**

```python
def _assemble_result(params: dict[str, Any], command: str, wall_time: float, tool_versions: dict[str, str | None], chain_cmds: dict[str, list[str]], chain_lengths: dict[str, int], final_convergence: dict[str, Any] | None, tool_outputs: dict[str, str], interrupted: bool, status: str = "success", error: str | None = None) -> dict[str, Any]:
    return {
        "status": status,
        "command": command,
        "wall_time": wall_time,
        ...
        "key_results": {
            ...
            "final_convergence": final_convergence or {},
...
def run_bi(
    ...
    if not dry_run:
        ...
        if resume is None:
            ...
            elif output_dir.exists() and any(output_dir.iterdir()) and not overwrite and not (output_dir / "run_state.json").exists():
    ...
    for name in names:
        if resume is None:
            chain_cmds[name] = _build_chain_cmd(tools["mpirun"], tools["pb_mpi"], threads, matrix, model_flags, sample_freq, nsamples, name)
        else:
    ...
    command = "phyloai tree bi " + " ".join(["--matrix", str(matrix), "--output-dir", str(output_dir), "--model", model, "--mixture", mixture, "--gamma-cats", str(gamma_cats), "--chains", str(chains), "--chain-prefix", chain_prefix, "--threads", str(threads), "--sample-freq", str(sample_freq), "--nsamples", str(nsamples), "--monitor-freq", str(monitor_freq), "--burnin-frac", str(burnin_frac)])
    if dry_run:
    ...
    return _run_bi_processes(output_dir, names, chain_cmds, tools, params, command, run_start, nsamples, monitor_freq, burnin_frac, no_plot, quiet)
```
...
def test_run_bi_dry_run_result_json_shape(tmp_path: Path):
    ...
    validate_params_completeness(payload, {"matrix", "output_dir", "overwrite", "model", "mixture", "gamma_cats", "start_tree", "fix_tree", "chains", "chain_prefix", "chain_names", "threads", "sample_freq", "nsamples", "resume", "monitor_freq", "burnin_frac", "no_plot", "pb_path", "dry_run", "quiet"})
```
...
```bash
git add phyloai/tree/bi.py tests/tree/test_bi.py
git commit -m "feat: add tree bi dry-run result assembly"
```
...
- [ ] **Step 1: Add soft-stop and process runner**
...
def _run_bi_processes(output_dir: Path, chain_names: list[str], chain_cmds: dict[str, list[str]], tools: dict[str, str], params: dict[str, Any], command: str, run_start: float, nsamples: int, monitor_freq: int, burnin_frac: float, no_plot: bool, quiet: bool) -> dict[str, Any]:
    ...
    try:
        ...
        while procs:
            ...
            if nsamples != -1:
                for name in list(procs):
                    if lengths.get(name, 0) >= nsamples:
                        (output_dir / "chains" / f"{name}.run").write_text("0\n")
            min_len = min(lengths.values()) if lengths else 0
            if min_len - last_check >= monitor_freq:
                burnin = math.floor(min_len * burnin_frac)
                if burnin >= 10:
                    final_convergence = _run_convergence_check(output_dir, chain_names, tools, burnin, no_plot)
                    last_check = min_len
            for name, proc in list(procs.items()):
                if proc.poll() is not None:
                    getattr(proc, "_phyloai_log").close()
                    if proc.returncode != 0:
                        return _assemble_result(params, command, time.monotonic() - run_start, {"pb_mpi": None, "bpcomp": None, "tracecomp": None, "mpirun": None}, chain_cmds, lengths, final_convergence, outputs, False, "error", f"pb_mpi chain {name} exited with code {proc.returncode}")
                    del procs[name]
        lengths = {name: _count_trace_samples(output_dir / "chains" / f"{name}.trace") for name in chain_names}
        if lengths:
        ...
        return _assemble_result(params, command, time.monotonic() - run_start, {"pb_mpi": None, "bpcomp": None, "tracecomp": None, "mpirun": None}, chain_cmds, lengths, final_convergence, outputs, False)
...
def test_run_bi_fake_tools_executes_chains(tmp_path: Path):
    ...
    matrix = tmp_path / "m.phy"
    matrix.write_text("2 3\na AAA\nb AAA\n")
    payload = run_bi(matrix=matrix, output_dir=tmp_path / "out", chains=2, nsamples=2, monitor_freq=1, burnin_frac=0.5, pb_path=tool_dir, no_plot=True, quiet=True)
    assert payload["status"] == "success"
...
```bash
git add phyloai/tree/bi.py tests/tree/test_bi.py
git commit -m "feat: add tree bi process execution"
```
...
def test_tree_bi_help_shows_flags():
    ...
    for flag in ["--matrix", "--model", "--mixture", "--gamma-cats", "--chains", "--threads", "--resume", "--pb-path"]:
...
```bash
git add phyloai/cli/commands/tree.py tests/cli/test_tree_bi.py
git commit -m "feat: register tree bi CLI command"
```
...
```bash
git add docs/commands/tree-bi.md docs/superpowers/specs/2026-06-07-phyloai-design.md docs/superpowers/specs/2026-06-17-phyloai-tree-design.md
git commit -m "docs: add tree bi command documentation"
```
...
- [ ] **Step 5: Commit verification-only docs if any changed**

If no files changed, do not commit. If docs were adjusted during verification:

```bash
git add docs/commands/tree-bi.md docs/superpowers/specs/2026-06-07-phyloai-design.md docs/superpowers/specs/2026-06-17-phyloai-tree-design.md
git commit -m "docs: finalize tree bi references"
```
...
- Spec coverage: CLI, model flags, chain naming, sampling, resume, run_state, pb-path, convergence commands, `.bpdiff` parsing, tracecomp parsing, plotting, output structure, result.json, doctor integration, and stale references are covered.
